...
Epd EpdReader::read(string epd) {
	int pos = StringUtils::nthOccurrence(epd, " ", 4);
	string epd_part1 = epd.substr(0, pos);
	string epd_part2 = epd.substr(pos + 1, epd.length());
	// split epd
	std::vector<string> splitted_epd_part1 = StringUtils::split(epd_part1, ' ');
	...
	std::vector<string> splitted_epd_part2 = StringUtils::split(epd_part2, ';');
